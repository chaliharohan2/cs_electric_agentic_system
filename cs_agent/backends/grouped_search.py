...
def group_path(hit: dict[str, Any], group_by: str) -> list[str]:
    """The path of the *group*, not of whichever member happened to be first.

    A family is a leaf, so its member's path is the group's path. A level is
    not: grouping by product_group put one arbitrary family's full path on the
    group — `Circuit Breakers` came back as `[..., 'Circuit Breakers',
    'Moulded Case Circuit Breakers', 'MCCB – Winbreak1']`. The prompts tell the
    model a path is a prefix it can reuse, so it would have scoped its next
    search into one sub-branch and silently dropped the other 1,100 SKUs the
    group counted.
    """
    path = list(hit.get("path") or [])
    if group_by == "family":
        return path
    depth = LEVEL_COLUMNS.index(group_by) + 1
    return path[:depth]
    ...
    *,
    ...
    for key in sorted(buckets):
        ...
        groups.append(
            {
                group_by: key,
                "path": group_path(members[0], group_by),
