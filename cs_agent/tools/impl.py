...
def _spec_rows(payload: Any) -> list[dict[str, Any]]:
...
def _known_families() -> tuple[str, ...]:
    ...
    names = {
        row.get("family")
        for row in _spec_rows(backend().list_canonical_specs())
        if row.get("family")
    }
...
def list_canonical_specs(
    ...
    if not isinstance(result, dict):
        result = {"specs": list(result), "by_spec_id": [], "scope": {"path": path, "family": family}}
